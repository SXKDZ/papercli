...
class CollectionDialog:
    ...
    def __init__(self, collections: List[Collection], papers: List[Paper], callback, status_bar=None):
        ...
        self.paper_moves = []  # [(paper_id, collection_name, action)] where action is 'add' or 'remove'
        ...
        self.papers_in_collection_list = EditableList(
            [], 
            on_select=self.on_paper_select,
            editable=False
        )
        
        all_paper_titles = [p.title[:50] + "..." if len(p.title) > 50 else p.title for p in papers]
        ...
        remove_button = Button(text="Remove →", handler=self.remove_paper_from_collection)
    ...
    def _create_dialog_key_bindings(self):
        ...
        def focus_next(event):
            self.current_focus_index = (self.current_focus_index + 1) % len(self.focusable_components)
    ...
    def on_collection_select(self, collection_name: str):
        ...
        if collection_name == "+ New Collection":
            ...
            self.other_papers_list.set_items([p.title[:50] + "..." if len(p.title) > 50 else p.title for p in self.all_papers])
            return
        
        # Find the selected collection
        self.current_collection = next((c for c in self.all_collections if c.name == collection_name), None)
        if not self.current_collection:
            return
            
        # Update papers lists
        papers_in_collection = [p.title[:50] + "..." if len(p.title) > 50 else p.title for p in self.current_collection.papers]
    ...
    def on_collection_edit(self, old_name: str, new_name: str):
        ...
        if old_name == "+ New Collection":
            ...
            if new_name not in [c.name for c in self.all_collections] and new_name not in self.new_collections:
                self.new_collections.append(new_name)
                # Update the collections list display
                current_names = [c.name for c in self.all_collections] + self.new_collections + ["+ New Collection"]
                self.collections_list.set_items(current_names)
        else:
            # Rename existing collection
            if new_name != old_name:
                # Check if new name already exists
                existing_names = [c.name for c in self.all_collections if c.name != old_name] + self.new_collections
                if new_name not in existing_names:
                    self.collection_changes[old_name] = new_name
                    # Update the collection in memory
                    collection = next((c for c in self.all_collections if c.name == old_name), None)
                    if collection:
    ...
    def add_paper_to_collection(self):
        ...
        if not paper_title or not collection_name or collection_name == "+ New Collection":
            return
    ...
    def save_changes(self):
        ...
        try:
            ...
            for old_name, new_name in self.collection_changes.items():
                success = self.collection_service.update_collection_name(old_name, new_name)
                if success:
                    saved_count += 1
                else:
                    error_count += 1
            ...
            for paper_id, collection_name, action in self.paper_moves:
                if action == 'add':
                    success = self.collection_service.add_paper_to_collection(paper_id, collection_name)
                else:  # remove
                    success = self.collection_service.remove_paper_from_collection(paper_id, collection_name)
            ...
            if self.status_bar:
                if error_count == 0:
                    if saved_count > 0:
                        self.status_bar.set_success(f"Successfully saved {saved_count} changes to collections")
                    else:
                        self.status_bar.set_status("No changes to save")
                else:
                    self.status_bar.set_warning(f"Saved {saved_count} changes, {error_count} errors occurred")
