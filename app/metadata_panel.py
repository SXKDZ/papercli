...
class MetadataPanel:
    ...
    def get_preview_text(self) -> FormattedText:
        ...
        # Header
        if len(self.papers) == 1:
            text.append(("class:header", f"Editing: {self.papers[0].title[:60]}...\n\n"))
        else:
        ...
        for i, (field_name, field_label) in enumerate(self.fields):
            ...
            if self.editing_field and i == self.current_field:
                text.append(("class:editing", "[EDITING - Press Enter to save, Escape to cancel]"))
            elif changed_value is not None:
                text.append(("class:changed", f"{changed_value} "))
                text.append(("class:original", f"(was: {current_value[:40]}...)"))
            else:
                text.append(("class:value", current_value[:60] + ("..." if len(current_value) > 60 else "")))
        ...
        if self.changes:
            text.append(("", "\n"))
            text.append(("class:warning", f"You have {len(self.changes)} unsaved changes"))
    ...
    def setup_layout(self):
        ...
        self.preview_window = Window(
            content=FormattedTextControl(
                text=lambda: self.get_preview_text()
            ),
            wrap_lines=True
        )
    ...
    def setup_key_bindings(self):
        ...
        def handle_enter(event):
            if self.editing_field:
                ...
                # Enter edit mode
                field_name, field_label = self.fields[self.current_field]
                current_value = self.changes.get(field_name, self.get_current_value(field_name))
        ...
        def save_changes(event):
            if self.changes:
                # Confirm changes
                changes_text = "\n".join([f"  {field}: {value}" for field, value in self.changes.items()])
