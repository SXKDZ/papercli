...
class DatabaseHealthService:
        ...
        try:
            with get_db_session() as session:
                ...
                try:
                    ...
                    checks["table_counts"]["collections"] = session.query(Collection).count()
                ...
                try:
                    ...
                    if result:
                        ...
                        self.issues_found.append(f"Foreign key constraint violations found: {len(result)} issues")
                except Exception as e:
    ...
    def _check_orphaned_records(self) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as session:
                ...
                orphaned_pc = session.execute(text("""
                ...
                orphaned["paper_collections"] = [(row[0], row[1]) for row in orphaned_pc]
                orphaned["summary"]["orphaned_paper_collections"] = len(orphaned_pc)
                
                # Check orphaned paper_authors
                orphaned_pa = session.execute(text("""
                ...
                if orphaned_pc:
                    self.issues_found.append(f"Found {len(orphaned_pc)} orphaned paper-collection associations")
                if orphaned_pa:
                    self.issues_found.append(f"Found {len(orphaned_pa)} orphaned paper-author associations")
    ...
    def _check_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(papercli_dir):
                ...
                if free_space < 100 * 1024 * 1024:  # Less than 100MB
                    self.issues_found.append("Low disk space: less than 100MB available")
        except Exception:
        ...
        try:
            papercli_dir = os.path.expanduser("~/.papercli")
            checks["permissions"]["papercli_dir_writable"] = os.access(papercli_dir, os.W_OK)
    ...
    def _check_terminal_setup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if size.columns < 80:
                self.issues_found.append("Terminal width is less than 80 columns (current: {})".format(size.columns))
            if size.lines < 24:
                self.issues_found.append("Terminal height is less than 24 lines (current: {})".format(size.lines))
        except Exception:
    ...
    def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if report["orphaned_records"]["summary"].get("orphaned_paper_collections", 0) > 0:
        ...
        try:
            with get_db_session() as session:
                ...
                if cleaned["paper_collections"] > 0:
                    self.fixes_applied.append(f"Cleaned {cleaned['paper_collections']} orphaned paper-collection associations")
                if cleaned["paper_authors"] > 0:
                    self.fixes_applied.append(f"Cleaned {cleaned['paper_authors']} orphaned paper-author associations")
