...
class SimpleFilterDialog:
    ...
    def show_filter_dialog(self, available_collections) -> Optional[Dict[str, Any]]:
        """Show filter dialog."""
        filters = {}
        ...
        # Year filter
        year_filter = self.dialogs.get_choice(
            "Filter by year:",
            [
                ("none", "No year filter"),
        ...
        if year_filter == "specific":
            year = self.dialogs.get_input("Enter year")
            if year:
                try:
            ...
            start_year = self.dialogs.get_input("Start year")
            end_year = self.dialogs.get_input("End year")
        ...
        # Venue filter
        venue = self.dialogs.get_input("Filter by venue (partial match, optional)")
        if venue:
            filters['venue'] = venue
        
        # Author filter
        author = self.dialogs.get_input("Filter by author (partial match, optional)")
        if author:
