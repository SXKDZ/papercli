...
from ng.services.database import DatabaseHealthService


class SyncOperation:
    """Represents a sync operation to be performed."""
    
    def __init__(self, operation_type: str, target: str, item_type: str, item_id: str, data: Dict = None):
        ...
        self.item_id = item_id  # title for papers, name for collections, filename for pdfs
...
class SyncResult:
    ...
    def get_summary(self) -> str:
        ...
        has_detailed_changes = any(self.detailed_changes.values()) if hasattr(self, "detailed_changes") else False
...
class SyncService:
    ...
    def __init__(self, local_data_dir: str, remote_data_dir: str, progress_callback=None, log_callback=None):
    ...
    def sync(self, conflict_resolver=None, auto_sync_mode=False) -> SyncResult:
        ...
        try:
            ...
            if conflicts and conflict_resolver:
                ...
                operations = self._resolve_conflicts_to_operations(conflicts, resolved_conflicts, operations)
            ...
            if self.log_callback:
                ...
                self.log_callback("sync_start", f"Starting remote→local sync: {len(remote_ops)} operations")
            ...
            if self.log_callback:
                ...
                self.log_callback("sync_start", f"Starting local→remote sync: {len(local_ops)} operations")
            ...
            if self.log_callback:
                self.log_callback("sync_start", "Starting collection synchronization...")
            ...
            if self.log_callback:
                self.log_callback("sync_error", f"Sync failed with error: {str(e)}")
        finally:
            self._release_locks()
            ...
            if title in remote_by_title:
                ...
                if self._papers_differ(local_paper, remote_paper):
                    operations.append(SyncOperation(
                        'conflict', 'both', 'paper', title, 
                        {'local': local_paper, 'remote': remote_paper}
                    ))
            else:
                operations.append(SyncOperation('add', 'remote', 'paper', title, local_paper))
        
        # Papers only in remote
        for title, remote_paper in remote_by_title.items():
            if title not in local_by_title:
                operations.append(SyncOperation('add', 'local', 'paper', title, remote_paper))
    ...
    def _generate_pdf_operations(self) -> List[SyncOperation]:
        ...
        remote_pdfs = {f.name: self._get_file_info(f) for f in self.remote_pdf_dir.glob("*.pdf")}
        ...
        for filename in set(local_pdfs.keys()) & set(remote_pdfs.keys()):
            ...
            if local_info.get('hash') != remote_info.get('hash'):
                operations.append(SyncOperation(
                    'conflict', 'both', 'pdf', filename,
                    {'local': local_info, 'remote': remote_info}
                ))
        
        # PDFs only in local
        for filename in local_pdfs.keys() - remote_pdfs.keys():
            operations.append(SyncOperation('add', 'remote', 'pdf', filename, local_pdfs[filename]))
        
        # PDFs only in remote
        for filename in remote_pdfs.keys() - local_pdfs.keys():
            operations.append(SyncOperation('add', 'local', 'pdf', filename, remote_pdfs[filename]))
            
        return operations

    def _operations_to_conflicts(self, conflict_operations: List[SyncOperation]) -> List[SyncConflict]:
        """Convert conflict operations to SyncConflict objects for UI compatibility."""
        conflicts = []
        for op in conflict_operations:
            conflict = SyncConflict(op.item_type, op.item_id, op.data['local'], op.data['remote'])
            conflicts.append(conflict)
        return conflicts

    def _resolve_conflicts_to_operations(self, conflicts: List[SyncOperation], 
                                       resolved_conflicts: Dict, 
                                       all_operations: List[SyncOperation]) -> List[SyncOperation]:
        """Convert user conflict resolutions back to concrete operations."""
        operations = []
        
        # Add non-conflict operations as-is
        operations.extend([op for op in all_operations if op.operation_type != 'conflict'])
        ...
        for conflict_op in conflicts:
            ...
            if resolution == 'local':
                ...
                if self.log_callback:
                    self.log_callback("sync_conflicts", f"Resolved conflict for {conflict_op.item_type} '{conflict_op.item_id}': using LOCAL version")
                operations.append(SyncOperation('delete', 'remote', conflict_op.item_type, conflict_op.item_id))
                operations.append(SyncOperation('add', 'remote', conflict_op.item_type, conflict_op.item_id, conflict_op.data['local']))
                
            elif resolution == 'remote':
                # Use remote version: delete local, add remote to local
                if self.log_callback:
                    self.log_callback("sync_conflicts", f"Resolved conflict for {conflict_op.item_type} '{conflict_op.item_id}': using REMOTE version")
                operations.append(SyncOperation('delete', 'local', conflict_op.item_type, conflict_op.item_id))
                operations.append(SyncOperation('add', 'local', conflict_op.item_type, conflict_op.item_id, conflict_op.data['remote']))
                
            elif resolution == 'keep_both':
                # Keep both: add both to both databases with different names
                if conflict_op.item_type == 'paper':
                    if self.log_callback:
                        self.log_callback("sync_conflicts", f"Resolved conflict for paper '{conflict_op.item_id}': KEEPING BOTH versions")
                    # Local version keeps original title
                    operations.append(SyncOperation('add', 'remote', 'paper', conflict_op.item_id, conflict_op.data['local']))
                    ...
                    if self.log_callback:
                        self.log_callback("sync_conflicts", f"Remote version renamed to: '{new_title}'")
                    operations.append(SyncOperation('delete', 'remote', 'paper', conflict_op.item_id))
                    operations.append(SyncOperation('add', 'remote', 'paper', new_title, remote_data))
                    operations.append(SyncOperation('add', 'local', 'paper', new_title, remote_data))
                
                elif conflict_op.item_type == 'pdf':
                    # Keep local PDF as-is, add remote with _remote suffix
                    if self.log_callback:
                        self.log_callback("sync_conflicts", f"Resolved conflict for PDF '{conflict_op.item_id}': KEEPING BOTH versions")
                    ...
                    if self.log_callback:
                        self.log_callback("sync_conflicts", f"Remote PDF renamed to: '{remote_filename}'")
                    operations.append(SyncOperation('add', 'local', 'pdf', remote_filename, conflict_op.data['remote']))
                    operations.append(SyncOperation('add', 'remote', 'pdf', remote_filename, conflict_op.data['remote']))
        
        return operations

    def _sync_remote_to_local(self, operations: List[SyncOperation], result: SyncResult):
            ...
            if op.target == 'local':
                if op.operation_type == 'add':
                    if op.item_type == 'paper':
                        ...
                        result.detailed_changes["papers_added"].append(f"'{op.item_id}' (from remote)")
                        # Log detailed paper information
                        if self.log_callback:
                            authors = op.data.get('authors', 'N/A')
                            venue = op.data.get('venue_full', 'N/A')
                            year = op.data.get('year', 'N/A')
                            self.log_callback("sync_remote_to_local", f"Added paper from remote: '{op.item_id}' by {authors} ({venue}, {year})")
                    elif op.item_type == 'pdf':
                        self._copy_pdf_file(self.remote_pdf_dir / op.item_id, self.local_pdf_dir / op.item_id)
                        result.changes_applied["pdfs_copied"] += 1
                        result.detailed_changes["pdfs_copied"].append(f"'{op.item_id}' (from remote)")
                        if self.log_callback:
                            self.log_callback("sync_remote_to_local", f"Copied PDF from remote: {op.item_id}")
                        
                elif op.operation_type == 'delete':
                    if op.item_type == 'paper':
                        self._delete_paper_by_title(self.local_db_path, op.item_id)
                        if self.log_callback:
                            self.log_callback("sync_remote_to_local", f"Deleted local paper: '{op.item_id}' (replaced by remote version)")
                    elif op.item_type == 'pdf':
                        pdf_path = self.local_pdf_dir / op.item_id
                        if pdf_path.exists():
                            pdf_path.unlink()
                            if self.log_callback:
                                self.log_callback("sync_remote_to_local", f"Deleted local PDF: {op.item_id} (replaced by remote version)")

    def _sync_local_to_remote(self, operations: List[SyncOperation], result: SyncResult):
            ...
            if op.target == 'remote':
                if op.operation_type == 'add':
                    if op.item_type == 'paper':
                        ...
                        result.changes_applied["papers_added"] += 1
                        result.detailed_changes["papers_added"].append(f"'{op.item_id}'")
                        # Log detailed paper information
                        if self.log_callback:
                            authors = op.data.get('authors', 'N/A')
                            venue = op.data.get('venue_full', 'N/A')
                            year = op.data.get('year', 'N/A')
                            self.log_callback("sync_local_to_remote", f"Added paper to remote: '{op.item_id}' by {authors} ({venue}, {year})")
                    elif op.item_type == 'pdf':
                        self._copy_pdf_file(self.local_pdf_dir / op.item_id, self.remote_pdf_dir / op.item_id)
                        result.changes_applied["pdfs_copied"] += 1
                        result.detailed_changes["pdfs_copied"].append(f"'{op.item_id}'")
                        if self.log_callback:
                            self.log_callback("sync_local_to_remote", f"Copied PDF to remote: {op.item_id}")
                        
                elif op.operation_type == 'delete':
                    if op.item_type == 'paper':
                        self._delete_paper_by_title(self.remote_db_path, op.item_id)
                        if self.log_callback:
                            self.log_callback("sync_local_to_remote", f"Deleted remote paper: '{op.item_id}' (replaced by local version)")
                    elif op.item_type == 'pdf':
                        pdf_path = self.remote_pdf_dir / op.item_id
                        if pdf_path.exists():
                            pdf_path.unlink()
                            if self.log_callback:
                                self.log_callback("sync_local_to_remote", f"Deleted remote PDF: {op.item_id} (replaced by local version)")
    ...
    def _sync_collections_by_timestamp(self, result: SyncResult):
        ...
        local_by_name = {col_data["name"]: (col_id, col_data) 
                        for col_id, col_data in local_collections.items()}
        remote_by_name = {col_data["name"]: (col_id, col_data) 
        ...
        for name in local_by_name.keys() - remote_by_name.keys():
            ...
            if self.log_callback:
                ...
                self.log_callback("sync_collections", f"Added collection to remote: '{name}' with {paper_count} paper(s)")
        ...
        for name in remote_by_name.keys() - local_by_name.keys():
            ...
            result.changes_applied["collections_added"] += 1
            result.detailed_changes["collections_added"].append(f"'{name}' (from remote)")
            if self.log_callback:
                paper_count = len(remote_papers)
                self.log_callback("sync_collections", f"Added collection from remote: '{name}' with {paper_count} paper(s)")
            ...
            if local_papers != remote_papers:
                ...
                for paper_title in list(all_papers):
                    ...
                    # Also check reverse mapping - if this is a remote version, include the original
                        ...
                        if original_title in all_papers or self._paper_exists_in_db(self.local_db_path, original_title):
                ...
                result.detailed_changes["collections_updated"].append(f"'{name}' (merged local and remote)")
                if self.log_callback:
                    ...
                    self.log_callback("sync_collections", f"Merged collection '{name}': local ({local_count}) + remote ({remote_count}) + keep_both ({keep_both_count}) = {merged_count} papers total")
    ...
    def _papers_differ(self, local_paper: Dict, remote_paper: Dict) -> bool:
        ...
        compare_fields = [
            "title", "abstract", "venue_full", "venue_acronym", "year", "volume", "issue", 
            "pages", "paper_type", "doi", "preprint_id", "category", "url", "notes"
        ]
    ...
    def _delete_paper_by_title(self, db_path: Path, title: str):
        ...
        try:
            ...
            if paper:
                ...
                cursor.execute("DELETE FROM paper_collections WHERE paper_id = ?", (paper_id,))
                cursor.execute("DELETE FROM papers WHERE id = ?", (paper_id,))
                conn.commit()
        finally:
            conn.close()

    def _copy_collection_to_local(self, collection_data: Dict, remote_collection_id: int):
        """Copy a collection from remote to local database."""
        conn = sqlite3.connect(self.local_db_path)
        cursor = conn.cursor()
        try:
            created_at = collection_data.get("created_at") or datetime.now().isoformat()
            cursor.execute("INSERT INTO collections (name, description, created_at, last_modified) VALUES (?, ?, ?, ?)",
                         (collection_data["name"], collection_data.get("description", ""), 
                          created_at, collection_data.get("last_modified")))
            new_collection_id = cursor.lastrowid
            
            # Copy paper relationships
            remote_papers = self._get_collection_papers(self.remote_db_path, remote_collection_id)
            for paper_title in remote_papers:
                ...
                if not paper_row and paper_title in self.title_mappings:
                    ...
                    cursor.execute("SELECT id FROM papers WHERE title = ?", (mapped_title,))
                    paper_row = cursor.fetchone()
                
                if paper_row:
                    cursor.execute("INSERT OR IGNORE INTO paper_collections (paper_id, collection_id) VALUES (?, ?)",
    ...
    def _copy_collection_to_remote(self, collection_data: Dict, local_collection_id: int):
        ...
        try:
            ...
            # Copy paper relationships
            local_papers = self._get_collection_papers(self.local_db_path, local_collection_id)
    ...
    def _is_local_collection_newer(self, local_modified, remote_modified):
        ...
        try:
            local_dt = datetime.fromisoformat(local_modified) if isinstance(local_modified, str) else local_modified
            remote_dt = datetime.fromisoformat(remote_modified) if isinstance(remote_modified, str) else remote_modified
    ...
    def _replace_collection_in_remote(self, local_collection_data, local_papers):
        ...
        try:
            ...
            cursor.execute("INSERT INTO collections (name, description, created_at, last_modified) VALUES (?, ?, ?, ?)",
                         (local_collection_data['name'], local_collection_data.get('description', ''),
                          local_collection_data.get('created_at'), local_collection_data.get('last_modified')))
            new_collection_id = cursor.lastrowid
    ...
    def _replace_collection_in_local(self, remote_collection_data, remote_papers):
        ...
        try:
            cursor.execute("DELETE FROM collections WHERE name = ?", (collection_name,))
            cursor.execute("INSERT INTO collections (name, description, created_at, last_modified) VALUES (?, ?, ?, ?)",
                         (remote_collection_data['name'], remote_collection_data.get('description', ''),
                          remote_collection_data.get('created_at'), remote_collection_data.get('last_modified')))
            new_collection_id = cursor.lastrowid
